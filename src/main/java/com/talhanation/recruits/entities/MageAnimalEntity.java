...
import com.talhanation.recruits.entities.ai.compat.RecruitAnimalRangedSpellAttackGoal;
...
public class MageAnimalEntity extends AbstractRecruitEntity {

    private final Predicate<ItemEntity> ALLOWED_ITEMS = (item) ->
            (!item.hasPickUpDelay() && item.isAlive() && getInventory().canAddItem(item.getItem()) && this.wantsToPickUp(item.getItem()));

    public MageAnimalEntity(EntityType<? extends AbstractRecruitEntity> entityType, Level world) {
    ...
    protected void registerGoals() {
        super.registerGoals();
        this.goalSelector.addGoal(1, new RecruitAnimalRangedSpellAttackGoal(this, 16.0D));
    }
    ...
    public void initSpawn() {
        this.setCustomName(Component.literal("AnimalMage"));
        ...
        this.setVariant(this.random.nextInt(100, 105));
    }
    ...
    public boolean wantsToPickUp(ItemStack itemStack) {
        ...
        if (Main.isSpellbooksLoaded) {
            if (AnimalStaffWeapon.isStaffItem(itemStack)) {
                return true;
            }
        }
