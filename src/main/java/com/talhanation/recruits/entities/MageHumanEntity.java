...
import com.talhanation.recruits.entities.ai.compat.RecruitHumanRangedSpellAttackGoal;
...
public class MageHumanEntity extends AbstractRecruitEntity {

    private final Predicate<ItemEntity> ALLOWED_ITEMS = (item) ->
            (!item.hasPickUpDelay() && item.isAlive() && getInventory().canAddItem(item.getItem()) && this.wantsToPickUp(item.getItem()));

    public MageHumanEntity(EntityType<? extends AbstractRecruitEntity> entityType, Level world) {
    ...
    protected void registerGoals() {
        super.registerGoals();
        this.goalSelector.addGoal(1, new RecruitHumanRangedSpellAttackGoal(this, 16.0D));
    }
    ...
    public void initSpawn() {
        this.setCustomName(Component.literal("HumanMage"));
        ...
        this.setVariant(this.random.nextInt(105, 110));
    }
    ...
    public boolean wantsToPickUp(ItemStack itemStack) {
        ...
        if (Main.isSpellbooksLoaded) {
            if (HumanStaffWeapon.isStaffItem(itemStack)) {
                return true;
            }
        }
