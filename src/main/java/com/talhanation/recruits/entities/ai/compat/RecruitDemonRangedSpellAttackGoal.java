...
import com.talhanation.recruits.compat.DemonStaffWeapon;
import com.talhanation.recruits.entities.MageDemonEntity;
...
public class RecruitDemonRangedSpellAttackGoal extends Goal {
    private final MageDemonEntity mage;
    private final double speedModifier;
    private int seeTime;
    private int attackTime = -1;
    private LivingEntity target;
    private final DemonStaffWeapon weapon;
    private final double stopRange;

    public RecruitDemonRangedSpellAttackGoal(MageDemonEntity mage, double stopRange) {
        this.weapon = new DemonStaffWeapon();
        this.mage = mage;
    ...
    protected boolean isStaffInHand() {
        ...
        return DemonStaffWeapon.isStaffItem(itemStack);
    }
    ...
    public void tick() {
        ...
        if (--this.attackTime <= 0 && this.seeTime >= -60) {
            ...
            // Cast level 5 fire projectile spell
            double x = target.getX();
            double y = target.getY() + target.getEyeHeight();
            double z = target.getZ();

            Main.LOGGER.debug("RecruitRangedSpellAttackGoal: About to cast spell at target " + target.getName() + " at position " + x + ", " + y + ", " + z);
            ...
            // Use weapon's spell casting method
            weapon.performRangedAttackIWeapon(mage, x, y, z, 1.5F);

            Main.LOGGER.debug("RecruitRangedSpellAttackGoal: Spell casting completed");
